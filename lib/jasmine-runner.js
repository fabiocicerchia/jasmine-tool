(function(){
  var exec = require('child_process').exec,
      ...
      Configuration = require(__dirname+'/config').Configuration,
  ...
  var commands = {
    ...
    run: function(){
      loadConfig(function(config){
        var server = require(__dirname+'/server').newServer(config);
        server.start();
      });
    },
    mon: function(info){
      loadConfig(function(config){
        ...
        if(info.provided_io) {
          config.provided_io = info.provided_io;
        }

        var server = require(__dirname+'/server').newServer(config);

        if(info.server) {
          server.app.use(info.server);
        }

        server.start();
        
        build_cmd = (typeof config.mon != 'undefined') ? config.mon.build_cmd : undefined;
        require(__dirname+'/monitor').startMonitor(server, build_cmd);
      });
    },
    ci:function(){
      loadConfig(function(config){
